...
export default function AddProductModal({ handleClose, open, categories }) {
  ...
  const isDisableSubmit = useMemo(
    () => !form.name || !form.description || !form.file || !form.cateId || form.amount <= 0,
    [form]
  );
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((url) => {
          productApi.createProduct({ ...form, image: url }).then((res) => {
            if (res.status === 204) {
              handleClose(true);
              setLoading(false);
            }
